...
<c:url value="/style/theme/images/admission_page_logo.jpg" var="StudentAddLogo" />
...
<script type="text/javascript">
				...
				.ajax({
					...
					success : function(data, status)
					{
						...
						else
						{
							...
							+ '<table class="table table-bordered table-primary" ><thead><tr><th colspan="6" class="head blue"><h4 style="text-align:center; padding:0px; margin:0px"> Issued Books Details</h4></th></tr><tr><th>Book Id</th><th>Book Name</th><th>Issue Date</th><th>Subject</th><th>ISBN</th><th>Action</th></tr></thead><tbody>';
							for (var i = 0; i < j.length; i++)
							{
								        st += '<tr>'
								        + '<td>' +j[i].issueBookId+ '</td>'
								        + '<td>' +j[i].bookName + '</td>'
										+ '<td>' + j[i].issueDate + '</td>'
										+ '<td>' + j[i].subjectName + '</td>'
										+ '<td>' + j[i].isbn + '</td>'
										+ '<td><button onclick="">Return</button><button>View</button></td></tr>';
							}
							st += '</tbody></table></div>';
						}
	...
	function getReturnBook(bookId)
	{
		...
		.ajax({
			type : "post",
			url : 'getReturnBookByBookId.html',
			data : 
			{
				'issueBookId' : bookId
			},
			success : function(data, status) 
			{
				...
				if (j == 0) 
				{
					 st = '<div class="failedmsg">Fail</div>';
				} 
				else
				{
					 st = '<div class="successmsg">Success</div>';
				}
	...
	function getIssuedByBookToStudent(bookId) {
		$("#issueBook").empty();
		$
				.ajax({
					type : "post",
					url : 'getIssuedBookByBookId.html',
					data : 
					{
						'bookId' : bookId
					},
					success : function(data, status) 
					{
						...
						else
						{
						    st =''
							+ '\n<div class="block" style="width:800px;">'
							+ '<table class="table table-bordered table-primary" ><thead><tr><th colspan="5" class="head blue"><h4 style="text-align:center; padding:0px; margin:0px"> Issued Books Details</h4></th></tr><tr><th>Book</th><th>Issue Date</th><th>Subject</th><th>ISBN</th><th>Return</th></tr></thead><tbody>';
							for (var i = 0; i < j.length; i++)
							{
								        st += '<tr>'
								        +'<td>' + j[i].bookName + '</td>'
...
<div id="content_wrapper">
	...
	<div id="content">
		...
		<br>
		<fieldset style="padding:0px; margin:0px; width:200px">
		<legend>Issue Book Detials</legend>
         <table class="table-condensed table-primary" >
         <thead>
         <tr><th colspan="2" class="success"></th></tr></thead>
         <tbody>
           <tr>
           
            <td>Student Id 
              <br>
                  <input type="text"  name="studentId" id="studentId" style="width: 190px; " onblur="getIssuedBookToStudent(this.value)">
            </td> 
           ...
           <fieldset style="padding:0px; margin:0px; width:200px ;"><legend>Return Book</legend>
           <f:form  action="returnExitBook.html" id="validate" method="post" name="myReturnForm" commandName="issueBookDTO">
		      <table class="table-condensed table-primary" >
		         <thead>
		         <tr><th colspan="2" class="success"></th></tr></thead>
		         <tbody>
		           ...
		           <tr>
		           <td>
		               <button type="submit" >Return</button><button type="button" >Cancel</button>
		           </td>
		           </tr>
		           </tfoot>
		          </table>

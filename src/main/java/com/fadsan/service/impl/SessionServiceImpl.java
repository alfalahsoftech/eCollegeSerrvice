...
public class SessionServiceImpl extends
	...
	public List<SessionDTO> findAllsession() {
		...
		final List<SessionDTO> list = new ArrayList<SessionDTO>();
		//Samdani
		sessionlist.stream().filter(session -> session.isActive()).forEach(sessDto ->{
			SessionDTO dto = new SessionDTO();
			dto.setSessionDuration(sessDto.getSessionDuration());
			dto.setSessionId(sessDto.getSessionId());
			list.add(dto);
		});
		/*
		 * for (Session session : sessionlist) { if (session.isActive() == true) {
		 * SessionDTO dto = new SessionDTO();
		 * dto.setSessionDuration(session.getSessionDuration());
		 * dto.setSessionId(session.getSessionId()); list.add(dto); } }
		 */
		return list;
	}
